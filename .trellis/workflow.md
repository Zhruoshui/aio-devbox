...
2. **Learn commit style**. Load the project's explicit commit convention first — `.trellis/spec/guides/commit-guidelines.md` if present (it OVERRIDES anything inferred from history when they conflict) — then skim recent history so drafted messages blend in:
   ```bash
   git log --oneline -5
   ```
