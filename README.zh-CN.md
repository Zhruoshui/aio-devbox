...
| `app` | `sandbox-app`(构建) | Axum 服务:托管 React SPA、`GET /api/manifest`(哪些按钮在线)、`/api/term/ws` pty WebSocket 桥、`POST/DELETE /api/buttons`(用户自注册按钮)、`/api/models/*`(模型配置页)、`/api/stats`。烘进 pi-web 时由 entrypoint 自启在 `:30141`。`FROM sandbox-base`。 |
...
三类自动探测按钮、用户自注册按钮、统一模型配置),以及 pi / pi-web agent 栈。
尚未做:按需 TUI 按钮之外的 L5 外部服务、自定义 web 型用户按钮(需跨容器端口
预览)——用户自注册按钮目前仅支持"终端+命令"。
